...
;(function($) {
	...
	$.fn.filedrop = function(options) {
		...
		function bindEvents(element) {
			element.bind('drop.filedrop', drop)
		...
		function releaseEvents() {
			$('#' + opts.fallback_id).unbind('change.filedrop');
			$(document).unbind('.filedrop');
			self.unbind('.filedrop');
		}
		...
		function getBuilder(filename, filedata, mime, boundary) {
			...
			builder += '; filename="' + encode_utf8(filename) + '"';
			builder += crlf;
		...
		//  }
			...
			var process = function() {
				...
				if (workQueue.length > 0) {
					process();
				//patch: for Issue #41 afterAll() don't run when allfiles are rejected
				} else {
				...
				xhr.onload = function() {
					if (xhr.responseText) {
						var now = new Date().getTime(),
							...
							result;
						//patch: accept non-JSON response
						if (opts.allowNonJSONResponse) {
							try {
								result = opts.uploadFinished(index, file, jQuery.parseJSON(xhr.responseText), timeDiff);
							} catch (e) {
								result = opts.uploadFinished(index, file, {'status': xhr.responseText}, timeDiff);
							}
						} else {
							result = opts.uploadFinished(index, file, jQuery.parseJSON(xhr.responseText), timeDiff);
						}
